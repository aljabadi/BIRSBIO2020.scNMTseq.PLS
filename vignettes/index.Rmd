...
# gastru.mae.imputed <- readRDS('gastru.mae.imputed.rds')
```
...
```{r, fig.width=8, fig.height=8}
plot_pls(pls_obj = mmspls.imputed, stage = gastru.mae.imputed$stage, col_palette = col_palette)
```
```{r, eval=params$save_output, echo=FALSE}
...
```{r}
concordance(pls_obj = mmspls.imputed, comp = c(1:4), which.plot = NULL)
```

Imputation does not considerably improve the method's performance as it inherently ignores the missing values during integration.
