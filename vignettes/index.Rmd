...
```{r perform nipals}
met_assays <- names(gastru.mae@ExperimentList)
nipals_comps <- list()
...
for (M in met_assays) {
  ...
  nipals_comps[[M]] <- 
    nipals::nipals(d, 
                   ...
                   maxiter = ifelse(params$mini_run, 2, 500))
}
...
```{r plot nipals, fig.width=8, fig.height=8}
## get scores
nipals_comps_df <- lapply(nipals_comps, function(y){
...
## plot
ggplot(nipals_comps_df, aes(PC1, PC2)) + geom_point(aes(col = stage)) +
...
## kmeans clustering balanced accuracy with PCA components from RNA data:
df <- data.frame(
  PCA_NIPALS = sapply(nipals_comps, function(x) {
