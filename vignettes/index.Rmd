...
**Note:** These analyses are (slightly) different from the ones presented in the hackathon in the sense that:
  ...
  - To enhance reproducibility on different machines, `uwot::umap` was used instead of `umap::umap` for dimensionality reduction of RNA data to avoid python dependencies. This will give different results in dimension reduction, but the population structure remains the same.
...
```{r}
cat(sprintf('Dropping cells from putative lineage(s) %s:\n', paste(sQuote(params$drop_lineages), collapse = ', ')))
...
df <- lapply(experiments(gastru.mae), function(w)
     ...
     c(
       n_cells = dim(w)[2],
       n_features = dim(w)[1],
       pct_missing = get_pct_missing(w)
     )
     })
df <- round(data.frame(df))
kable(df, align = 'l')
```
...
```{r}
mmspls <- multimodal_sPLS_wrapper(mae = gastru.mae, study_assays = NULL, ncomp = 2, scale = FALSE, design = 'null', lineages = NULL, stages = NULL, DA = NULL, keepX = NULL, save = FALSE)
```
