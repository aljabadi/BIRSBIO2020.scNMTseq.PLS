...
params:
  ## setup params
  on_my_mac: !r Sys.info()['user'] == 'ablabadi'
  save_output: false
  local_data: false
  mini_run: false
...
# remotes::install_github('mixOmicsTeam/mixOmics@MultiAssayExperiment', upgrade = 'never',force = TRUE)
```
...
Load the `MultiAssayExperiment` object using [SingleCellMultiModal](https://bioconductor.org/packages/release/data/experiment/html/SingleCellMultiModal.html) package:

```{r, eval=!params$on_my_mac}
# cat(sprintf('loading the following assays from ExperimentHub: %s\n', paste(params$keep_assays, collapse = ', ')))
cat('loading the assay data from ExperimentHub: %s\n')
gastru.mae <- scNMT(dataType = 'mouse_gastrulation', modes = '*', version = '2.0.0', dry.run = FALSE, verbose = FALSE)
```
