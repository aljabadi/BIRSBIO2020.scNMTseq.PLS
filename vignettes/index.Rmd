...
# Apply NIPALS for PCA with all assays

We apply NIPALS dimension reduction algorithm on all data:


```{r perform nipals, warning=FALSE}
...
```{r, ech=FALSE}
## helper function to create nipals plots coloured by stage
plot_nipals_redcuedDims <- function(nipals_comps_df, red_dim = 'PC', col_palette = col_palette) {
  axes <- paste0(red_dim, '_', 1:2)
  ggplot(nipals_comps_df, aes_string(axes[1], axes[2])) + geom_point(aes(col = stage)) +
    scale_color_manual(values = col_palette) + facet_wrap(.~Modality, scales = 'free', ncol = 2) +
    theme_classic() +  theme(legend.position = c(0.93, .05),
...
Plot PCs:
```{r plot nipals PCs, fig.width=8, fig.height=8}
plot_nipals_redcuedDims(nipals_comps_df, red_dim = 'PC', col_palette = col_palette)
```
...
We finally calculate the balanced prediction accuracy of a kmeans clustering algorith. We use the first 2 components from PCA components from individual assays as wel as PLS variates from integration of original and imputed measurements:
...
```{r}
set.seed(427)
## kmeans clustering balanced accuracy with PCA components from RNA data:
kmeans_accuracy(variates = rna.sce.matching@int_colData$reducedDims@listData$PCA[,1:2], labels = mmspls$stage)
pca_comps <- c(list(rna = list(scores=rna.sce.matching@int_colData$reducedDims@listData$PCA)), nipals_comps)
## kmeans clustering balanced accuracy with PCA components from RNA data:
df <- data.frame(
  PCA = sapply(pca_comps, function(x) {
        kmeans_accuracy(variates = x$scores[,1:2], labels = mmspls$stage)
    }),
  ## without imputation
    PLS_noImpute = sapply(mmspls$variates, function(x) {
        kmeans_accuracy(variates = x[,1:2], labels = mmspls$stage)
    }),
    ## with imputation
    PLS_impute = sapply(mmspls.imputed$variates, function(x) {
        kmeans_accuracy(variates = x[,1:2], labels = mmspls$stage)
    })
)
...
The shared axes of variation across assays are more predictive of the embryonic stage than PCA components derived using Expectation Maximization PCA.

Without imputation, with the exception of genebody methylation, the components from all assays are nearly equally predictive. After imputation, the accuracy of the prediction is improved particularly for genebody methylation components.

The integrative components show stronger phenotypic relevance in terms of embryonic stage for genebody and CGI methylation measurements. This was also apparent from score plots where different stages were better separated for these views after imputation.
